...
"""
Generate synthetic questions and answers for FRIDGR images. Input is a single
...
slots; by convention <Z> = Size, <C> = Category, <M> = Freshness, <S> = Shape.
...
expand into a combination of "size", "filter_category", "filter_freshness",
...
# Inputs
parser.add_argument('--input_scene_file', default='../fridgr-output/FRIDGR_scenes.json',
    help="JSON file containing ground-truth scene information for all images " +
         "from render_images.py")
parser.add_argument('--metadata_file', default='fridgr_metadata.json',
    help="JSON file containing metadata about functions")
parser.add_argument('--synonyms_json', default='fridgr_synonyms.json',
    help="JSON file defining synonyms for parameter values")
parser.add_argument('--template_dir', default='FRIDGR_0.1_templates',
    help="Directory containing JSON templates for questions")

# Output
parser.add_argument('--output_questions_file',
    default='../fridgr-output/FRIDGR_questions.json',
...
parser.add_argument('--templates_per_image', default=2, type=int,
    help="The number of different templates that should be instantiated " +
         "on each image")
parser.add_argument('--instances_per_template', default=10000, type=int,
    help="The number of times each template should be instantiated on an image")

# Misc
parser.add_argument('--reset_counts_every', default=10000, type=int,
...
def precompute_filter_options(scene_struct, metadata):
  # Keys are tuples (size, freshness, category, shape) (where some may be None)
  # and values are lists of object idxs that match the filter criterion
  attribute_map = {}
  global FRIDGR_DATASET
  if metadata['dataset'] == FRIDGR_DATASET:
    attr_keys = ['size', 'freshness', 'category', 'shape']
  else:
  ...
  masks.append([1,1,0,1])
  masks.append([1,1,1,0])
  #print('objects:', len(scene_struct['objects']))
...
'''def add_empty_filter_options(attribute_map, metadata, num_to_add):
  ...
  target_size = len(attribute_map) + num_to_add*len(masks)
  print('\nattr_vals', len(attr_vals), attr_vals)
  print('\nattribute_map', type(attribute_map), len(attribute_map), attribute_map)
  while len(attribute_map) < target_size:
    ...
    #  attribute_map[k] = []
    for mask in masks:
      #k = (random.choice(v) for m,v in enumerate(attr_vals) if mask[m] == 1 else None)
      for m,v in enumerate(attr_vals):

        k = (random.choice(v) )
      print(len(attribute_map), mask, k)
      #if k not in attribute_map:
      #  attribute_map[k] = []
  print('attribute_map2', target_size, len(attribute_map), attribute_map)'''
...
def add_empty_filter_options(attribute_map, metadata, num_to_add):
  ...
  #print('\nattr_vals', len(attr_vals), attr_vals)
  #print('\nattribute_map', target_size, len(attribute_map), attribute_map)
  while len(attribute_map) < target_size:
    for mask in masks:
        attr_vals = [metadata['types'][t] if mask[m] == 1 else [None] for m,t in enumerate(attr_keys)]
        #print('ATTR =', attr_vals)
        if '_filter_options' in metadata:
            attr_vals = metadata['_filter_options']
...
def instantiate_templates_dfs(scene_struct, template, metadata, answer_counts,
  ...
  while states:
    ...
    if state['next_template_node'] == len(template['nodes']):
      ...
      #print('ANSWER:', answer_counts, answer)
      ...
      for k in filter_option_keys:
        ...
        for param_name, param_val in zip(filter_side_inputs, k):
          ...
          if param_val is not None:
            ...
            if metadata['dataset'] == FRIDGR_DATASET and param_type == 'Shape' and catNone:
              #print(param_val, catNone)
              param_val = 'thing'
            else:
              #print(param_val)
              param_val = ''
...
def main(args):
  ...
  # change the name to "value_inputs" for the public FRIDGR release. I should
  ...
  # in some of the code above; however in the public FRIDGR release all functions
