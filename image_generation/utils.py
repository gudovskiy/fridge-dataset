...
def add_object(object_dir, name, scale, loc, theta=0, file=""):
  """
  ...
  if file != "":
    filename = os.path.join(object_dir, file, 'Object', name)
  else:
  ...
  print(name, new_name, filename)
  bpy.data.objects[name].name = new_name

  #scale so xy dimensions are about '2'
  #scale /= 0.5*max(bpy.data.objects[new_name].dimensions[:2])
  ...
  #position so lowest z-coord is on the ground
  z = min([(bpy.data.objects[new_name].matrix_world * mathutils.Vector(v)).z for v in bpy.data.objects[new_name].bound_box])
  bpy.data.objects[new_name].location.z -= z
